...
# Version   |   Description
#------------------------------------------------------------------
#   1.3        Fixed a bug in the display update logic that was causing the display to not update correctly.
#   1.2        Modified to work with the Raspberry Pi 5. Updated the serial port to /dev/ttyAMA0.
#   1.1        Added functionality to send status updates to the TemperatureServer over the serial port every 30 seconds.
#                   Added functionality to manage the LCD display to show the current date and time, the current temperature, and the state of the thermostat along with its set temperature.
#                   Added functionality to manage the LED indicators based on the current state of the thermostat and the current temperature relative to the setpoint.
#                   Added functionality to manage the buttons to cycle through the states of the thermostat and to increase or decrease the setpoint by a degree.
...
##
ser = serial.Serial(
        port='/dev/ttyAMA0', # This would be /dev/ttyAM0 prior to Raspberry Pi 3
                                # Changed to /dev/ttyAMA0 for RPi 5
...
class TemperatureMachine(StateMachine):
    ...
    def on_enter_heat(self):
        ## Turn the red light on when we enter the heat state.
        redLight.pulse()
    ...
    def on_exit_heat(self):
        ## Turn the red light off when we exit the heat state.
        redLight.off()
    ...
    def on_enter_cool(self):
        ## Turn the blue light on when we enter the cool state.
        blueLight.pulse()
    ...
    def on_exit_cool(self):
        ## Turn the blue light off when we exit the cool state.
        blueLight.off()
    ...
    def on_enter_off(self):
        ## Turn both lights off when we enter the off state.
        redLight.off()
        blueLight.off()
    ...
    def processTempStateButton(self):
        ...
        ## Transition the state machine to the next state in the cycle. This will
        ## cause the on_exit and on_enter methods to be called for the appropriate
        ## states.
        self.send('cycle')
    ...
    def processTempIncButton(self):
        ...
        ## Increases the temperature setpoint by one degree
        self.setPoint += 1
        ## Update the status lights immediately to reflect the new setpoint
        self.updateLights()
    ...
    def processTempDecButton(self):
        ...
        ## Decreases the temperature setpoint by one degree
        self.setPoint -= 1
        ## Update the status lights immediately to reflect the new setpoint
        self.updateLights()
    ...
    def updateLights(self):
        ...
        if(DEBUG):
            print(f"Mode: {self.current_state.id}")
            print(f"Set: {self.setPoint}")
            print(f"Temp: {temp}")

        # Determine visual identifiers

        ## Update the lights based on the current state of the thermostat and the 
        #  current temperature relative to the setpoint.
    ...
    def setupSerialOutput(self):
        ## Creates the string that will be sent to the TemperatureServer over the Serial Port (UART). 
        #  This is a comma delimited string indicating the current state of the thermostat, 
        #  the temperature in degrees Fahrenheit, and the current setpoint of the thermostat - 
        #  also in degrees Fahrenheit.

        output = f"{self.current_state.id[0].upper()},{floor(self.getFahrenheit())},{self.setPoint}"
        ## ^ Modified to send only the first letter of the state in uppercase to fit within the 16x2 display charcter limit
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            #  First line of the display will show the current date and time in the format MM/DD/YY HH:MM
            #  Shortened year format to YY to fit within the 16x2 display character limit
            #  Seconds removed to fit within the 16x2 display character limit
            self.lcd_line_1 = current_time.strftime("%m/%d/%y %H:%M")

    
            ## Setup Display Line 2
            if(altCounter < 6):
                ## Second line of the display will show the current temperature in degrees Fahrenheit
                self.lcd_line_2 = f"Temp: {floor(self.getFahrenheit())} F"

                # Increment the timer for line 2 to alternate between the two display lines every 5 seconds    
                altCounter = altCounter + 1
            else:
                ## Second line of the display will show the current state of the thermostat and the setpoint in degrees Fahrenheit
                ## Modified to show only the first letter of the state in uppercase to fit within the 16x2 display character limit
                self.lcd_line_2 = f"Mode:{self.current_state.id[0].upper()} Set:{self.setPoint}F"
            ...
            ## Update Display with lines separated by a newline character. This will cause the display to show the first line on the first row and the second line on the second row.
            screen.updateScreen(self.lcd_line_1 + "\n" + self.lcd_line_2)
    
            ## Update server every 30 seconds
            if(DEBUG):
               print(f"Counter: {counter}")
            if((counter % 30) == 0):
                ## Use UART to send the current state of the thermostat, the current temperature in degrees Fahrenheit, and the setpoint of the thermostat in degrees Fahrenheit to the TemperatureServer.
...
## Assign the processTempStateButton method to be executed when the button is pressed
...
##
redButton = Button(25)
## Assign the processTempIncButton method to be executed when the button is pressed
redButton.when_pressed = tsm.processTempIncButton


##
## Configure our Blue button to use GPIO 21 and to execute
## the function to decrease the setpoint by a degree.
##
blueButton = Button(21)
## Assign the processTempDecButton method to be executed when the button is pressed
