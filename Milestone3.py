...
# Version   |   Description
#------------------------------------------------------------------
#   1.3        Fixed bug in the on_enter_letterPause and on_enter_wordPause methods where the state 
#               machine was not returning to the off state after the pause.
#   1.2        Completed the TODO items in the code to implement the full functionality of the 
#               state machine.
#   1.1        Updated the messages to be sent to be HELLO and WORLD instead of SOS and OK. 
#               This is to make the demonstration more interesting, and to allow for more 
#               thorough testing of the state machine functionality. 
...
class CWMachine(StateMachine):
    ...
    def on_enter_dot(self):
        ...
        # Return to off state for next event
        self.doDot()
    ...
    def toggleMessage(self):
        ## Check which message is active, and switch to the other one
        match self.activeMessage:
            case self.message1:
